...
struct Contraption_t {
	...
	/* 0x180 */ char pad_004[0x20]; // this is genuinely random in the game (we are coa  
	/* 0x1A0 */ HWND hWnd; // this is random in the game at first but managed
};

#ifdef NDEBUG
static char pad[0x1267538 - 0x1DC30] = { 0 }; // offset it to replicate the game's memory layout
#else
...
int main() {
	...
	// Hook LoadLibraryW so we can detect and block DLL injection (we need to block it
	// because since we aren't the real game so mods could crash, this is only for testing,
	// so seeing the mods injecting isn't important)
	MH_Initialize();
	MH_CreateHook(&LoadLibraryW, (LPVOID)(LoadLibraryW_t)[](LPCWSTR lpLibFileName)->HMODULE {
		std::wstring libFileName(lpLibFileName);
		std::string libFileNameStr(libFileName.begin(), libFileName.end());

		// Check if `Carbon` is in the DLL name
		if (libFileNameStr.find("Carbon") != std::string::npos) {
			spdlog::info("Allowing DLL injection of {}", libFileNameStr);
			return oLoadLibraryW(lpLibFileName);
		}

		spdlog::trace("Blocking DLL injection of {}", libFileNameStr);
		return nullptr;
	}, (LPVOID*)&oLoadLibraryW);

	MH_EnableHook(&LoadLibraryW);
	spdlog::info("Allowing one DLL injection...");
	...
	spdlog::info("Game state type: {}", Contraption->gameStateType);
